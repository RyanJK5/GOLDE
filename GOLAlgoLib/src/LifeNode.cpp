...
uint64_t LifeNode::ComputeHash(const LifeNode* nw, const LifeNode* ne,
    ...
    const auto a = std::bit_cast<uint64_t>(nw) >> 4;
    const auto b = std::bit_cast<uint64_t>(ne) >> 4;
    const auto c = std::bit_cast<uint64_t>(sw) >> 4;
    const auto d = std::bit_cast<uint64_t>(se) >> 4;

    // Fast 4-to-1 mix using rotations + xor-fold + splitmix64 finalizer.
    auto hash = a ^ std::rotl(b, 16) ^ std::rotl(c, 32) ^ std::rotl(d, 48);
